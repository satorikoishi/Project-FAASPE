...
def add_trace_oracle_metrics(rows, output_dir):
    oracle = next((row for row in rows if row["variant"] == "trace-oracle"), None)
    if oracle is None:
        return rows
    ...
    for row in rows:
        metric = dict(row)
        metric["oracle_variant"] = "trace-oracle"
        ...
        for idx in range(count):
            oracle_side = record_side(oracle_records[idx], "trace-oracle")
...
def run_trace(args, output_dir, container_result_dir, model):
    ...
    for variant in variants:
        ...
        log_name = f"invocations_trace_{variant}.jsonl"
        params = {
            ...
            "ACCESS": "hot",
            "TRACE_FILE": "/usr/src/app/depth_trace.csv",
    ...
    write_csv(output_dir / "placement_trace_latency_summary.csv", rows)
    metric_rows = add_trace_oracle_metrics(rows, output_dir)
    write_csv(output_dir / "placement_trace_oracle_metrics.csv", metric_rows)
    return metric_rows
