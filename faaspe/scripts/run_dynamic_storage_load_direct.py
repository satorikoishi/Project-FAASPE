...
FUNC_NAME = "dynamic-storage-load"
DEFAULT_TRACE_PATH = ROOT / "functions" / FUNC_NAME / "dynamic_storage_load_trace.csv"
DEFAULT_VARIANTS = "local,remote,faaspe,static-only,arbiter-only,runtime-only,storage-load-oracle"
HEARTBEAT_VARIANTS = {"faaspe", "runtime-only"}
...
def add_oracle_metrics(rows, output_dir):
    oracle = next((row for row in rows if row["variant"] == "storage-load-oracle"), None)
    if oracle is None:
        return rows

    oracle_log = output_dir / oracle.get("source_invocation_log", "")
    oracle_records = read_jsonl(oracle_log)
    oracle_total = float(oracle.get("total_time_s") or 0)
    metric_rows = []
    for row in rows:
        metric = dict(row)
        metric["oracle_variant"] = "storage-load-oracle"
        ...
        for idx in range(count):
            oracle_side = record_side(oracle_records[idx], "storage-load-oracle")
...
def heartbeat_env_for_variant(variant, args):
    if variant not in HEARTBEAT_VARIANTS:
        return {
            "FAASPE_STORAGE_HEARTBEAT_ENABLED": "0",
            "FAASPE_STORAGE_HEARTBEAT_MANUAL": "0",
        }
    return {
        "FAASPE_STORAGE_HEARTBEAT_ENABLED": "1",
        "FAASPE_STORAGE_HEARTBEAT_MANUAL": "1",
        "STORAGE_LOAD_HEARTBEAT_STEP": str(args.heartbeat_step),
    }


def run_workload(args, output_dir, container_result_dir, model):
    rows = []
    ...
    output_trace_path = output_dir / "dynamic_storage_load_trace.csv"
    shutil.copyfile(trace_path, output_trace_path)
    docker_cp_to_container(FUNC_NAME, output_trace_path, "/usr/src/app/dynamic_storage_load_trace.csv")
    ...
    for variant in variants:
        ...
        log_name = f"invocations_dynamic_storage_load_{variant}.jsonl"
        params = {
            ...
            "PUSH_ADDR": PUSH_ADDR,
            "PULL_ADDR": PULL_ADDR,
            "VALUE_LEN": args.object_size,
            "KEY_COUNT": args.key_count,
            "ACCESS": "hot",
            "TRACE_FILE": "/usr/src/app/dynamic_storage_load_trace.csv",
        ...
        params.update(heartbeat_env_for_variant(variant, args))
        ...
        if args.restart_function_container:
            ...
            docker_cp_to_container(
                FUNC_NAME,
                output_trace_path,
                "/usr/src/app/dynamic_storage_load_trace.csv",
            )

        invoke(FUNC_NAME, params)
        file_name = f"dynamic_storage_load_{variant}.csv"
        ...
        rows.append(
            {
                ...
                "heartbeat_enabled": result.get("storage_heartbeat_enabled", ""),
                "heartbeat_samples": result.get("storage_heartbeat_samples", ""),
                "heartbeat_estimated_load_us": result.get("storage_heartbeat_estimated_load_us", ""),
                "heartbeat_observed_latency_us": result.get("storage_heartbeat_observed_latency_us", ""),
    ...
    write_csv(output_dir / "dynamic_storage_load_latency_summary.csv", rows)
    metric_rows = add_oracle_metrics(rows, output_dir)
    write_csv(output_dir / "dynamic_storage_load_oracle_metrics.csv", metric_rows)
    return metric_rows


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", default="dynamic-storage-load-direct")
    ...
    parser.add_argument("--heartbeat-step", type=int, default=5)
    ...
    rows = run_workload(args, output_dir, container_result_dir, model)
    print("DYNAMIC_STORAGE_LOAD_SUMMARY_CSV")
    for row in rows:
        print(
            "{variant},{median_ms},{native_count},{func_count},{heartbeat_enabled},{heartbeat_samples},{placement_correct_rate},{normalized_total_latency},{normalized_regret}".format(
                **row
            )
        )
